    ...
    populatePoll(poll_id) {
        ...
        let populatePoll = {
            ...
            body: JSON.stringify({
                poll_id: poll_id,
                numMovies:
                    (this.state.numMovies != "") ?
                    parseInt(this.state.numMovies) :
                    null,
                minYear: 
                    (this.state.minYear != "") ?
                ...
                maxYear: 
                    ...
                    null,
                minRuntime: 
                    (this.state.minRuntime!= "") ?
                    parseInt(this.state.minRuntime) :
                    null,
                maxRuntime: 
                    (this.state.maxRuntime != "") ?
                    parseInt(this.state.maxRuntime) :
                    null,
                ratingTomato: 
                    (this.state.critic.value == "Rotten Tomatoes") ?
                    rating :
                    null,
                ratingMeta: 
                    (this.state.critic.value == "Metacritic") ?
                    rating :
                    null,
                ratingIMDB: 
                    (this.state.critic.value == "IMDB") ?
                    rating :
                    null,
                ratingParent: 
    ...
    render() {
        ...
        return (
            <View style={{flex: 1, justifyContent: 'Center', alignItems: 'center'}}>
                ...
                />
                <Text>on:</Text>
                <Dropdown options={critics} onChange={(critic) => this.setState({critic})} value={this.state.critic.value} />
                <Text>Parental Rating</Text>
                <Dropdown options={parentalRatings} onChange={(ratingParent) => this.setState({ratingParent})} value={this.state.ratingParent.value} />
                <Text>Genre:</Text>
                <Dropdown options={this.state.all_genres} onChange={(genre) => this.setState({genre})} value={this.state.genre.value} />
